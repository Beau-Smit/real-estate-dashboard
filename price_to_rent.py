...
df_chi_home_zip = pd.concat(
    [df_chi_home[["RegionName"]], df_chi_home_recent], axis=1
).set_index("RegionName")
df_chi_rent_zip = pd.concat(
    [df_chi_rent[["RegionName"]], df_chi_rent_recent], axis=1
).set_index("RegionName")
...
df_long = pd.melt(
    df_chi_ratio.reset_index(),
    id_vars=["RegionName"],
    value_vars=df_chi_ratio.columns,
    var_name="Date",
)
